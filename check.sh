...
case "$TARGET" in
  windows-x86_64)
    ...
    for file in "$CHECK"/*.exe "$CHECK"/*.dll; do
      dlls="$(x86_64-w64-mingw32-objdump -p "$file" | awk '/DLL Name:/ { print tolower($3) }' | sort -u)"
      echo "$(basename "$file") imports: $(echo $dlls)"
      unexpected="$(echo "$dlls" | grep -vE "^($own|$system)$" || true)"
      [ -z "$unexpected" ] || fail "unexpected dependency in $(basename "$file"): $unexpected"
    done
    ;;
  ...
  macos-arm64)
    for file in "$CHECK"/ffmpeg "$CHECK"/ffprobe "$CHECK"/*.dylib; do
      ...
      otool -L "$file" | tail -n +2
      unexpected="$(otool -L "$file" | tail -n +2 | awk '{ print $1 }' \
    ...
    ./ffmpeg -hide_banner -L | grep -q 'GNU Lesser General Public License' || fail "the build is not LGPL"
    ...
    for encoder in libvpx-vp9 libvpx aac libmp3lame libopus flac mpeg4 h264_videotoolbox hevc_videotoolbox; do
      echo "$encoders" | grep -qw -- "$encoder" || fail "encoder $encoder is missing"
    done
    ...
    for decoder in libdav1d h264 hevc vp9 mpeg4 wmv3 prores aac opus amrnb adpcm_ms wmav2; do
      echo "$decoders" | grep -qw -- "$decoder" || fail "decoder $decoder is missing"
    done
    ...
    for file in silence.mp3 silence.m4a silence.opus silence.mkv silence-16k.wav grey.avi grey.webm $produced; do
      printf '%-24s ' "$file"
      ./ffprobe -v error -show_entries stream=codec_name,sample_rate,width,height:format=format_name,duration \
